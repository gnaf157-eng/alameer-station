...
public class Db extends SQLiteOpenHelper {
    ...
    public void updatePump(long id,String name,String fuel,double price,double reading,long workerId){SQLiteDatabase db=getWritableDatabase();ContentValues v=new ContentValues();v.put("name",name.trim());v.put("fuel",fuel.trim());v.put("price",price);v.put("last_reading",reading);v.put("worker_id",workerId);db.update("pumps",v,"id=?",new String[]{String.valueOf(id)});ContentValues r=new ContentValues();r.put("previous",reading);r.put("price",price);db.update("readings",r,"pump_id=? AND current IS NULL",new String[]{String.valueOf(id)});}
    public double fuelPrice(String fuel){try(Cursor c=getReadableDatabase().rawQuery("SELECT price FROM pumps WHERE fuel=? ORDER BY id LIMIT 1",new String[]{fuel})){return c.moveToFirst()?c.getDouble(0):0;}}
    public void setFuelPrices(double diesel,double petrol,double gas){SQLiteDatabase db=getWritableDatabase();db.beginTransaction();try{setFuelPrice(db,"ديزل",diesel);setFuelPrice(db,"بترول",petrol);setFuelPrice(db,"غاز",gas);db.setTransactionSuccessful();}finally{db.endTransaction();}}
    private void setFuelPrice(SQLiteDatabase db,String fuel,double price){if(price<=0)return;ContentValues p=new ContentValues();p.put("price",price);db.update("pumps",p,"fuel=?",new String[]{fuel});db.execSQL("UPDATE readings SET price=? WHERE current IS NULL AND pump_id IN (SELECT id FROM pumps WHERE fuel=?)",new Object[]{price,fuel});}
    ...
    public long openShift(int workerId, boolean night) {
        ...
        try(Cursor c=db.rawQuery("SELECT id FROM shifts WHERE worker_id=? AND status IN ('OPEN','SUBMITTED') ORDER BY id DESC LIMIT 1",new String[]{String.valueOf(workerId)})){if(c.moveToFirst())return c.getLong(0);}
