...
public class ArchiveActivity extends Activity {
    @Override public void onCreate(Bundle b){super.onCreate(b);Db db=new Db(this);int workerId=getIntent().getIntExtra("workerId",0);boolean admin=getIntent().getBooleanExtra("admin",false);ScrollView scroll=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(18,24,18,40);root.setBackgroundColor(Color.rgb(247,250,252));root.addView(Util.title(this,admin?"أرشيف جميع الورديات":"أرشيف وردياتي"));try(Cursor c=db.archive(workerId,admin)){if(c.getCount()==0)root.addView(Util.label(this,"لا توجد ورديات محفوظة بعد."));while(c.moveToNext()){double bal=c.getDouble(5);TextView card=Util.label(this,"وردية #"+c.getLong(0)+" — "+c.getString(1)+"\n"+c.getString(2)+"\nالحالة: "+status(c.getString(3))+" | المبيعات: "+fmt(c.getDouble(4))+"\nالباقي: "+fmt(bal)+" | المزامنة: "+sync(c.getString(6)));card.setBackgroundColor(Color.WHITE);card.setPadding(20,20,20,20);root.addView(card,new LinearLayout.LayoutParams(-1,-2));}}scroll.addView(root);setContentView(scroll);}
