...
public class CashboxFlowTest {
 ...
 @Test public void editingApprovedOpeningPostsOnlyAuditedDifference(){
  ...
  review();CashAccounts.setOpening(db,box,"YER",1500);assertEquals(1,ShiftWorkspace.reviewed(db,shift));refuse(()->ShiftWorkspace.review(db,shift,1));
 }
 ...
 @Test public void foreignTransferAndReceiptUseTheirSelectedCurrencies(){
  ...
  review();db.closeAndPostShift(shift,db.soloWorkerId(),"",box);assertEquals(6,CashAccounts.posted(db,box,"SAR"),0.00001);assertEquals(1,CashAccounts.posted(db,other,"USD"),0.00001);assertEquals(db.journalDebit(),db.journalCredit(),0.00001);
 }
 @Test public void upgradeFrom22PreservesAmountsAndCurrencyCounts(){
  ...
  assertEquals(23,db.getReadableDatabase().getVersion());assertEquals(1000,db.cashboxBalance(sar),0.00001);assertEquals(10,CashAccounts.posted(db,sar,"SAR"),0.00001);assertEquals(8,CashAccounts.expected(db,shift,sar,"SAR"),0.00001);assertEquals(8,ShiftWorkspace.counted(db,shift,1,CashAccounts.key(sar,"SAR")),0.00001);
 }
 @Test public void cardStaysOpenForRepeatedEntriesAndShowsLiveBalances(){
  ...
  org.robolectric.android.controller.ActivityController<ShiftActivity> controller=Robolectric.buildActivity(ShiftActivity.class).setup();ShiftActivity a=controller.get();a.workspacePage(5);WorkspaceForms host=new WorkspaceForms(a,a.pages[5],1);CashEntryCard card=new CashEntryCard(host,2);card.show();card.from.setSelection(card.boxes.ids.indexOf(box));Shadows.shadowOf(android.os.Looper.getMainLooper()).idle();card.currency.setSelection(1);Shadows.shadowOf(android.os.Looper.getMainLooper()).idle();
  card.category.setText("كهرباء");card.amount.setText("3");assertEquals("10 سعودي",card.current.getText().toString());assertEquals("7 سعودي",card.after.getText().toString());card.dialog.getButton(-1).performClick();assertTrue(card.dialog.isShowing());assertEquals(box,card.boxes.id(card.from));assertEquals("SAR",card.code());assertEquals("",card.amount.getText().toString());assertEquals("7 سعودي",card.current.getText().toString());
  card.chooseKind(0);card.person.setText("عميل");card.amount.setText("2");assertEquals("9 سعودي",card.after.getText().toString());card.dialog.getButton(-1).performClick();assertTrue(card.dialog.isShowing());assertEquals(2,rows("shift_operations"));assertEquals("9 سعودي",card.current.getText().toString());assertEquals(0,rows("cashbox_entries"));card.dialog.getButton(-2).performClick();assertFalse(card.dialog.isShowing());controller.pause().stop().destroy();
 }
}
