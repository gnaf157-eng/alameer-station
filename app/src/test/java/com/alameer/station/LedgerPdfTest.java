...
public class LedgerPdfTest {
 ...
 @Test public void longNotesContinueAcrossPagesWithoutClippingAndSelectedPagesAreRespected()throws Exception{
  String longNote="نقل وقود إلى المحطة ومراجعة الكمية وأجرة السائق ".repeat(90)+"نهاية البيان الطويل";debt("DEBT",25,longNote);for(int i=0;i<62;i++)debt(i%2==0?"DEBT":"PAID",10,"حركة رقم "+(i+1));
  ...
  File selected=new File("build/reports/pdf/ledger-selected-page.pdf");try(FileOutputStream out=new FileOutputStream(selected)){PageRange[] written=report.write(out,new PageRange[]{new PageRange(1,1)},new CancellationSignal(),null);assertEquals(1,written.length);assertEquals(new PageRange(1,1),written[0]);}
  ...
  try(FileOutputStream out=new FileOutputStream(new File("build/reports/pdf/ledger-landscape.pdf"))){report.write(out,new PageRange[]{new PageRange(0,0)},new CancellationSignal(),landscape);}
 }
